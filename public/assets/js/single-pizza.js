...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      // check for a 4xx or 5xx error from server, session
      if (!response.ok) {
    ...
    .then(printPizza)
    // .catch() so that any error takes the user back to the home page, using the window.history.back() method
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza with this id! Taking you back.');
      // below will expose methods to allow control of the state of the browser's session
     // like a `back` button
      window.history.back();
    });
}
